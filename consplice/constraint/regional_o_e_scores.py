...
error_log_file = file_date + "regional_o_and_e_scores.ERROR.log_count_matrix.ERROR.log"
out_log_file = file_date + "regional_o_and_e_scores.OUT.log"
...
def add_regional_o_e(sub_p):
    ...
    req.add_argument(
        ...
        metavar="Window Size",
        required = True,
        help="The window size in bp used for the sliding window. Region from a gene will be created using this window size"
    )
...
def regional_o_e(parser, args):
    ...
    print("\tGetting substitution rates")
    ...
    #pbar.set_description("Sliding window along gene & adding tracking structure")
    ...
    transcript_quarter = len(transcript_region_dict) / 4

    tx_quantile_index = np.percentile(np.arange(0,len(transcript_region_dict)),[0,25,50,75,100])

    transcript_keys = list(transcript_region_dict.keys())


    for tx_key in transcript_region_dict.keys():
        
        ## Create a region per gene dict
        regional_transcript_region_dict = defaultdict(lambda: defaultdict(str))

        regions = sliding_window_regions(start        = transcript_region_dict[tx_key]["gene_start"],
                                         end          = transcript_region_dict[tx_key]["gene_end"],
        ...
        for region_start, region_end in regions:
            ...
            regional_transcript_region_dict[region_key] = copy.deepcopy(transcript_region_dict[tx_key])
            regional_transcript_region_dict[region_key]["start"] = region_start
            regional_transcript_region_dict[region_key]["end"] = region_end

            ## Add error tracking per transcript
            regional_transcript_region_dict[region_key]["non_matching_ref_alelles"] = 0.0
            regional_transcript_region_dict[region_key]["non_matching_symbol"] = 0.0
            regional_transcript_region_dict[region_key]["multiple_ref_allele_pos"] = 0.0
            regional_transcript_region_dict[region_key]["ref_alleles_as_N"] = 0.0
            regional_transcript_region_dict[region_key]["empty_position"] = 0.0
            regional_transcript_region_dict[region_key]["seg_dup_pos"] = 0.0
            regional_transcript_region_dict[region_key]["self_chain_pos"] = 0.0
            regional_transcript_region_dict[region_key]["positions_considered"] = 0.0
            regional_transcript_region_dict[region_key]["total_gene_positions"] = regional_transcript_region_dict[region_key]["gene_end"] - regional_transcript_region_dict[region_key]["gene_start"] + 1
            regional_transcript_region_dict[region_key]["total_region_positions"] = region_end - region_start + 1


        #pbar.close()

        ## Remove the original transcript dict
        ##del transcript_region_dict


        #print("\n\nProcessing transcript regions") 
        #print("=============================\n") 
        #pbar = tqdm(total = len(regional_transcript_region_dict))
        #pbar.set_description("Processing transcript regions")


        
        ## iterate over each region and get o and e scores
        dict_keys = list(regional_transcript_region_dict.keys())
        for transcript_key in dict_keys:
            ...
            for label in delta_score_bin_list:
                ...
                regional_transcript_region_dict[transcript_key]["{}_zeroton_observed".format(label)] = 1.0
                regional_transcript_region_dict[transcript_key]["{}_zero_and_singleton_observed".format(label)] = 1.0
                regional_transcript_region_dict[transcript_key]["{}_zeroton_expected".format(label)] = 1.0
                regional_transcript_region_dict[transcript_key]["{}_zero_and_singleton_expected".format(label)] = 1.0

                ## Delta score bin count
                regional_transcript_region_dict[transcript_key]["{}_count".format(label)] = 0.0
                ...
                for ref_allele in ["A","C","G","T"]:
                    ...
                    regional_transcript_region_dict[transcript_key]["{}_{}_zeroton_observed".format(label,ref_allele)] = 1.0
                    regional_transcript_region_dict[transcript_key]["{}_{}_zero_and_singleton_observed".format(label,ref_allele)] = 1.0
                    regional_transcript_region_dict[transcript_key]["{}_{}_zeroton_expected".format(label,ref_allele)] = 1.0
                    regional_transcript_region_dict[transcript_key]["{}_{}_zero_and_singleton_expected".format(label,ref_allele)] = 1.0  

            ## Add general observed and expected score counts
            regional_transcript_region_dict[transcript_key]["zeroton_observed_var_count"] = 0.0
            regional_transcript_region_dict[transcript_key]["zero_and_singleton_observed_var_count"] = 0.0
            regional_transcript_region_dict[transcript_key]["zeroton_expectation_sum"] = 0.0
            regional_transcript_region_dict[transcript_key]["zero_and_singleton_expectation_sum"] = 0.0



            ## Get region info
            chrom = regional_transcript_region_dict[transcript_key]["chrom"]
            start = regional_transcript_region_dict[transcript_key]["start"]
            end = regional_transcript_region_dict[transcript_key]["end"]
            strand = regional_transcript_region_dict[transcript_key]["strand"]
            gene_name = regional_transcript_region_dict[transcript_key]["gene_name"] 
            ...
            for i,spliceai_var in enumerate(spliceai_vcf("{}:{}-{}".format(chrom,start,end))):
                ...
                if list(segdup_interlap_dict[chrom].find((spliceai_var.POS, spliceai_var.POS))): 
                    ...
                    regional_transcript_region_dict[transcript_key]["seg_dup_pos"] +=  1.0  
                    continue

                elif list(selfchain_interlap_dict[chrom].find((spliceai_var.POS, spliceai_var.POS))):
                    seg_dup_or_self_chain += 1
                    regional_transcript_region_dict[transcript_key]["self_chain_pos"] += 1.0 
                    continue
            ...
            regional_transcript_region_dict[transcript_key]["multiple_ref_allele_pos"] = len(multi_ref_pos)
            ...
            for relative_spliceai_region in spliceai_region_list:
                ...
                (non_matching_ref_alleles, 
                 ...
                 empty_position_info) = get_gnomad_vars_by_position(gnomad_vcf,
                                                                    ...
                                                                    alt_symbol_dict,
                                                                    regional_transcript_region_dict,
                                                                    transcript_key,
            ...
            with open(args.out_file, "a") as out: 
                ...
                value = regional_transcript_region_dict.pop(transcript_key)
