...
def add_aggregate_overlapping_regions(sub_p):
    ...
    p.add_argument(
        ...
        default="Aggregated_ConSplice_Percentile",
        help="(Optional) The name of the new percentile score column that is created. Default = 'Aggregated_ConSplice_Percentile'",
    )
...
def create_interlap_object(
    ...
    Method to create an interlap object, interval tree with little overhead. This interlap object will take start and end positions
     from a pandas dataframe and create an interlap object for each interval. Interlap objects will be separated into chromosomes using
    ...
    4) tx_start_col:    (str) The name of the column in the dataframe that has represents the start positions of the transcript
    5) tx_end_col:      (str) The name of the column in the dataframe that has represents the end positions of the transcript
    ...
    Returns:
    ++++++++
    1) (dict) A dictionary of interlap objects. Keys = chromosome names. Values = Interlap object created from intervals in the dataframe with the associated chromosome.
    """
    ...
    for row in df.itertuples():
        ...
        interlap_dict[getattr(row, chrom_col)].add(
            (
                int(getattr(row, start_col)),
...
def aggregate_intersecting_regions(
    chrom, start_pos, end_pos, strand, symbol, step_size, interlap_dict, agg_type
):
    """
    aggregate_intersecting_regions
    ==============================
    Method to get the overlapping/intersecting interval from an interlap object and different regions in a gene and aggregate the scores for
    ...
    6) step_size:     (int)  The size of regions to create across the gene/region
    ...
    while temp_end < end_pos:
        ...
        for intersect in interlap_dict[chrom].find((temp_start, temp_end)):
            ...
            ## 1) the strand of the intersect object is the same as the region of interest
            ...
            if (
                intersect[4] == strand
                and intersect[7] == symbol
                and intersect[1] != temp_start
            ):
        ...
        if o_over_e_values:
            new_regions.append(
                [
                    ...
                    symbol,
                    step_size,
                    np.mean(o_over_e_values)
                    if agg_type == "mean"
                    else np.median(o_over_e_values),
                ]
            )
...
def agg_overlapping_regions(parser, args):
    ...
    inter_dict = create_interlap_object(
        scores_df,
        "Chromosome",
        "region_start",
        "region_end",
    ...
    ## Iterate over each gene and get the new step size regions with the aggregated score
    step_regions = []
    for i, row in enumerate(
        scores_df.groupby(["gene_id", "transcript_id"]).first().itertuples()
    ):

        step_regions.extend(
            aggregate_intersecting_regions(
                chrom=row.Chromosome,
                start_pos=int(row.txStart),
                end_pos=int(row.txEnd),
                strand=row.strand,
                symbol=row.gene_symbol,
                step_size=int(args.step_size),
                interlap_dict=inter_dict,
            ),
    ...
    new_region_w_pctl_df = convert_scores_to_percentiles(
        query_df=new_regions_df,
    ...
    new_region_w_pctl_df = new_region_w_pctl_df.sort_values(
        by=["#chrom", "region_start"]
    )

    print("\n> Writing output to '{}'".format(args.out_file))
