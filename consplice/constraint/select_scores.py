...
def select_scores(parser, args):
    ...
    print(
        "\n\tKeeping the '{}' O/E and '{}' Percentile score fields".format(
            args.o_over_e_field, args.pctl_field
        )
    )

    print(
        "\n\tRenaming the '{}' O/E score field to '{}".format(
            args.o_over_e_field, args.new_oe_name
        )
    )

    print(
        "\n\tRenaming the '{}' Percentile score field to '{}'".format(
            args.pctl_field, args.new_pctl_name
        )
    )
    ...
    scores_df["region_size"] = scores_df.region_end - (
        scores_df.region_start - 1
    )  ## offset by 1 to count for the first position and last position, not just the difference
