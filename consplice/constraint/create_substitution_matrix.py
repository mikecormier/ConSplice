...
def add_substitution_matrix(sub_p):
    ...
    req.add_argument(
        ...
        required=True,
        help="(Required) The gnomAD coverage file for the gnomAD vcf file. The coverage files should be in a bed formatted, bgzipped, and tabixed.",
    )
    ...
    req.add_argument(
        ...
        metavar="GTF file",
        required=True,
        help="(Required) A gtf file to get gene specific genomic coordinates from to build the count matrix.",
    )
...
def get_merged_gtf_regions(
    gtf_file, feature="transcript", biotype="protein_coding", cpus=3
):
    """
    get_merged_gtf_regions
    ======================
    Method to get regions from a gtf file based on feature type and biotype of that feature. By default,
     all regions that are protein-coding transcripts will be returned. All overlapping regions are merged into a
    ...
    3) biotype:  (str) The gene's biotpye to filter for. Only features with this gene biotype will be returned. (Default = protein_coding)
    ...
    biotype_column = (
        "gene_biotype"
        if "gene_biotype" in gtf_pr_columns
        else "gene_type"
        if "gene_type" in gtf_pr_columns
        else None
    )

    ## Filter gtf by gene feature and biotype to get a dict of genes with strand info
    feature_gene_gtf_pr = gtf_pr.subset(
        lambda x: (x.Feature == "gene") & (x[biotype_column] == biotype)
    )
    t_gene_df = (
        feature_gene_gtf_pr.df[["gene_name", "gene_id", "Strand"]]
        .rename(columns={"Strand": "strand"})
        .transpose()
    )
    ...
    filtered_gtf_pr = gtf_pr.subset(
        lambda x: (x.Feature == feature) & (x[biotype_column] == biotype)
    )

    ## merge overlapping feature regions based on strand. That is, only merge regions that are on the same strand
    merged_pr = filtered_gtf_pr.merge(
        count=True, count_col="Merged_Features", strand=True
    )
...
def create_mut_freq_table(d_score_bins):
    ...
    Method to create a mutation frequency table for each of the SpliceAI delta bins and different ref/alt combinations.
     This is a raw table with default values. This table will be added to as the mutation observations occur.
    ...
    ## Get a list of AC count dicst for the number of unique splice region labels
    ac_type_list = {"AC<1": 0, "AC<2": 0, "AC=1": 0, "AC>0": 0, "AC>1": 0}
    ...
    for ref_to_alt in [[x, y] for x in alleles for y in alleles]:
        ...
        mutation_dict[ref_to_alt[0]][ref_to_alt[1]] = {
            delta_score_bin: copy.deepcopy(ac_type_list)
            for delta_score_bin in d_score_bins
        }

    return mutation_dict


def get_gnomad_vars_by_position(
    vcf,
    contains_chr,
    region_chrom,
    region_start,
    ...
    Method to get gnomAD variants by position and update the mutation frequency table.
    It takes a cyvcf2 object and iterates over each variant that is found between the position interval. Checks are done
     for each variant found to ensure the variant is appropriate. Checks include:
        ...
        5) If the variant has a matching annotated symbol. If not, variant is skipped
    ...
    12) vep_index:                (list) A list that represents the vep index in the gnomad vcf
    ...
    17) non_matching_symbols:     (int)  The current count of non matching symbols
    ...
    ## Iterate through each position in the region and substitution counts
    for region_pos in spliceai_region_dict.keys():
        ...
        for pos_dict in pos_list:
            ...
            ## Get position info
            z = pos_dict["zerotons"]
            zps = pos_dict["zerotons_plus_singletons"]
            s = pos_dict["singletons"]
            nz = pos_dict["non_zerotons"]
            nzps = pos_dict["non_zerotons_plus_singletons"]
            score_bin = pos_dict["delta_score_bin"]
            corrected_ref = pos_dict["strand_corrected_ref"]
            corrected_alt = pos_dict["strand_corrected_alt"]

            ## Add count to dict
            mutation_count_table[corrected_ref][corrected_alt][score_bin]["AC<1"] += z
            mutation_count_table[corrected_ref][corrected_alt][score_bin]["AC<2"] += zps
            mutation_count_table[corrected_ref][corrected_alt][score_bin]["AC=1"] += s
            mutation_count_table[corrected_ref][corrected_alt][score_bin]["AC>0"] += nz
            mutation_count_table[corrected_ref][corrected_alt][score_bin][
                "AC>1"
            ] += nzps
...
def write_mutation_count_table(mutation_count_table, output_file):
    ...
    Parameters:
    -----------
    1) mutation_count_table: (dict) The count table for variants per ref/alt/delta bin
    ...
    with open(output_file, "w") as out:
        ...
        out.write(
            "#ref\talt\tdelta_score_bin\tzerotons\tzerotons_plus_singletons\tsingletons\tnon_zerotons\tnon_zerotons_plus_singletons\n"
        )
...
def substitution_matrix(parser, args):
    ...
    substitution_matrix
    ===================
    Main method used to create the splicing aware substitution matrix for the ConSplice Model
    """
    ...
    if args.cc > 1.0 or args.cc < 0.0:
        print(
            "\n!!ERROR!! The Coverage cutoff is set at {}. It needs to be between 0.0 and 1.0. Please correct the problem and try again".format(
                args.cc
            )
        )
        sys.exit(1)
    ...
    ## Additionally, get a dict of gene symbols as keys and a dict with "Strand" info as a value key
    regions_df, gene_strand_dict = get_merged_gtf_regions(
        args.gtf_file, feature="transcript", biotype="protein_coding", cpus=ncpus
    )
    print("\nQuery Regions:")
    print(regions_df)
    ...
    for index, region in enumerate(regions_df.itertuples()):
        ...
        ## reload vcf to control memory usage
        if prev_chrom != chrom:
            spliceai_vcf = VCF(args.spliceai_vcf, threads=ncpus)
            gnomad_vcf = VCF(args.gnomad_vcf, threads=ncpus)
            prev_chrom = chrom
        ...
        if chrom == "X":
            ...
            nonrepeat_regions = (
                np.concatenate(new_regions) if len(new_regions) > 0 else np.array([[]])
            )
        ...
        ).format(region, spliceai_region_list)
    ...
    print(
        "\nWriting mutation count table by reference allele to '{}'".format(
            args.out_file
        )
    )
